...
def test_massive_webpage():
    # Jeremy 9/18/24: increasing timeout for all of these API calls to one minute, since the server's responsiveness fluctuates greatly but as long as it's up we can tolerate delays
    requests.get("http://massive.ucsd.edu/ProteoSAFe/datasets.jsp", timeout=60).raise_for_status() #Datasets Page
    requests.get("http://massive.ucsd.edu/ProteoSAFe/dataset.jsp?task=fd246a746e0749c5ad0403be265bb2ea", timeout=60).raise_for_status() #Dataset Page
    requests.get("http://massive.ucsd.edu/ProteoSAFe/MassiveServlet?function=reanalysis&task=fd246a746e0749c5ad0403be265bb2ea", timeout=60).raise_for_status() #Reanalysese
    requests.get("http://massive.ucsd.edu/ProteoSAFe/MassiveServlet?function=massivehistory&massiveid=MSV000079514", timeout=60).raise_for_status()
    requests.get("http://massive.ucsd.edu/ProteoSAFe/MassiveServlet?function=massivesummary&massiveid=MSV000079514", timeout=60).raise_for_status()
