...
if CONFIG_FILE is not None and CONFIG_FILE.strip() != "":
    ...
    print("test_massive_api: using specified configuration file [" + CONFIG_FILE + "]")
else:
    CONFIG_FILE = os.path.join(CONFIG_DIRECTORY, "production.cfg")
    print("test_massive_api: configuration file not specified - using default configuration file [" + CONFIG_FILE + "]")
TARGET_WEB_SERVERS = []
TARGET_FTP_SERVERS = []
    ...
    with open(CONFIG_FILE, "r") as file_reader:
        ...
        for line in file_reader:
            ...
            # unrecognized section for this test suite
                ...
                elif mode == "ftp" and line not in TARGET_FTP_SERVERS:
                    TARGET_FTP_SERVERS.append(line)
    print("test_massive_api: using test targets from configuration file: web [" + ", ".join(TARGET_WEB_SERVERS) + "] / FTP [" + ", ".join(TARGET_FTP_SERVERS) + "]")
else:
